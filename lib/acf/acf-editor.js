...
function learnMapFromRow($row) {
    ...
    if (valB && valB !== 'none' && valA) {
        ...
        if (targetVal === 'all_oppo') {
            ...
            if (hitCountVal >= 2) {
                availableMaps = window.kotoMaps.multiSingleMaps;
            } else {
        ...
        for (var i = 0; i < availableMaps.length; i++) {
            ...
            var compareVal = Array.isArray(mapVal) ? mapVal[0] : mapVal;
            if (compareVal == valA) {
...
function autoFillRow($row, selectedType) {
    ...
    if (targetVal === 'all_oppo') {
        ...
        if (hitCountVal >= 2) {
            currentAvailableMaps = window.kotoMaps.multiSingleMaps;
        } else {
    ...
    if (activeMap && activeMap[selectedType] !== undefined) {
        ...
        if (Array.isArray(mapData)) {
            ...
            if (targetHitCount && mapData[0] !== undefined && mapData[0] !== '') {
                targetHitCount.val(mapData[0]);
            }
            if (targetAttackValue && mapData[1] !== undefined && mapData[1] !== '') {
                targetAttackValue.val(mapData[1]);
            }
            if (targetAttackValueLast && mapData[2] !== undefined && mapData[2] !== '') {
...
acf.addAction('new_field/name=waza_value', function (field) {
    field.on('change', function () {
        console.log('✅ waza_value changed! New value:', field.val());
...
acf.addAction('new_field/name=waza_value_last', function (field) {
    field.on('change', function () {
        console.log('✅ waza_value_last changed! New value:', field.val());
