...
(function ($) {
    ...
    $(document).on('click', '#btn_auto_input_fill', function (e) {
        e.preventDefault();

        // 入力されたテキストの収集
        // ★修正1: 配列 [] ではなく オブジェクト {} にする
        var texts = {};

        $('.acf-auto-input-text').each(function () {
            var val = $(this).val().trim();
            var id = $(this).attr('id');
            var key=id.replace('auto_input', ''); // IDからフィールドタイプを抽出（例: "waza_auto_input_textarea" → "waza"）
            if (val && id) {
                // ★修正2: オブジェクトのキーにIDを指定して、値を格納する
                texts[id] = val;
            }
        });

        console.log('送信するテキスト:', texts);

        // ★修正3: オブジェクトが空かどうかの判定方法を変更
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'koto_parse_auto_input',
                texts: texts
            },
            success: function (response) {
                if (response.success) {
                    console.log('成功:', response.data);
                    // ACFの繰り返しフィールドへの挿入処理（第2段階で実装）
                } else {
                    console.error('パースエラー(PHP側でエラー)');
                }
            },
            // ★追加: PHPにすら辿り着けなかった時のエラーログ
            error: function (xhr, status, error) {
                console.error('AJAX通信エラー:', error);
                console.log(xhr.responseText); // サーバーから返ってきた生のエラー文を見る
            }
        });
    });
})(jQuery);
...
function hasAttackType($row) {
    var typeField = acf.getField($row.find('.acf-field[data-name="type"]'));
    if (!typeField) typeField = acf.getField($row.find('.acf-field[data-name="waza_type"]'));
    if (typeField) {
        var typeVal = typeField.val();
        if (Array.isArray(typeVal)) {
            return typeVal.includes('attack');
        } else if (typeof typeVal === 'string') {
            return typeVal.indexOf('attack') !== -1;
        }
    }
    return true; // フィールドが存在しない場合は安全側に倒してtrue（従来の挙動を維持）
}
