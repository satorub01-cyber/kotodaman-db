...
(function ($) {
    $(document).ready(function () {
        ...
        $(document).on('keydown', function (e) {
            ...
            if (e.ctrlKey && e.altKey && e.shiftKey && e.key.toLowerCase() === 'm') {
                // querySelector で最初の1つを取得してクリック
                var mapBtn = document.getElementById('koto-show-map-btn') || document.querySelector('#koto-show-map-btn');
                if (mapBtn) {
                    mapBtn.click();
                }
            }
        ...
        if (typeof acf !== 'undefined') {
            acf.addAction('append', function ($el) {
                ...
                if ($el.hasClass('acf-row')) {
                    setTimeout(function () {
                        var $firstInput = $el.find('.acf-accordion-title, button:not([disabled]), div[tabindex], input:not([type="hidden"]):not([disabled]):not([readonly]), select:not([disabled]):not([readonly]), textarea:not([disabled]):not([readonly])').first();
                        if ($firstInput.length) {
                            if (!['INPUT', 'SELECT', 'TEXTAREA', 'BUTTON'].includes($firstInput[0].tagName) && !$firstInput[0].hasAttribute('tabindex')) {
                                $firstInput[0].setAttribute('tabindex', '-1');
                            }
    ...
    $(document).on('click', '.acf-auto-input-header', function () {
        var $content = $(this).siblings('.acf-auto-input-content');
        if ($content.is(':visible')) {
            $content.hide();
        } else {
            // 親に gap や align-items が設定されているため flex で表示する
...
jQuery(document).ready(function ($) {
    ...
    window.kotoMaps = {
        ...
        multiRandomMaps: [
            ...
            { 'name': 'ラッシュ', 'strong': ['4', '0.6', '1.2'], 'very_strong': ['', '', ''], 'super_strong': ['6', '1.1', '2.5'], 'most_strong': ['12', '0.9', '1.1'] },
        ]
    };
    ...
    // =========================================================
    setTimeout(function () {
        var $firstInput = $('.acf-editor-main-form').find('.acf-accordion-title, button:not([disabled]), div[tabindex], input:not([type="hidden"]):not([disabled]):not([readonly]), select:not([disabled]):not([readonly]), textarea:not([disabled]):not([readonly])').first();
        if ($firstInput.length) {
            if (!['INPUT', 'SELECT', 'TEXTAREA', 'BUTTON'].includes($firstInput[0].tagName) && !$firstInput[0].hasAttribute('tabindex')) {
                $firstInput[0].setAttribute('tabindex', '-1');
            }
...
// タイプにattackが含まれるか判定するヘルパー関数
function hasAttackType($row) {
    var typeFields = [
        acf.getField($row.find('.acf-field[data-name="type"]')),
        acf.getField($row.find('.acf-field[data-name="waza_type"]')),
        acf.getField($row.find('.acf-field[data-name="trait_type"]'))
    ];
    var hasTypeField = false;
    ...
    for (var i = 0; i < typeFields.length; i++) {
        ...
        if (Array.isArray(typeVal)) {
            if (typeVal.includes('attack')) {
                return true;
            }
        } else if (typeof typeVal === 'string') {
            if (typeVal.indexOf('attack') !== -1) {
                return true;
            }
        }
    }

    return !hasTypeField ? true : false; // type系フィールドがない場合は従来挙動を維持
}

// 行の情報からMapを推測し、学習（保存）するグローバル関数
function learnMapFromRow($row) {
    if (!$row || !$row.length) return false;
    if (!hasAttackType($row)) return false;
    var $ = window.jQuery;
...
// =================================================================
// ★修正＆機能追加：手動Map確認ボタンをモーダル表示化
// =================================================================
document.addEventListener('click', function (event) {
    var btn = event.target.closest('#koto-show-map-btn');
    if (!btn) return;

    // 1. 存在チェック
    if (!window.kotoMaps || typeof window.kotoMaps !== 'object') {
        console.error('🗺️ Error: window.kotoMaps is not defined or not an object.');
        return;
    }

    console.log('🗺️ All Maps:', window.kotoMaps);

    // 2. モーダル用の要素（外枠・背景）を作成、または取得
    var modal = document.getElementById('koto-map-modal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'koto-map-modal';

        // モーダル全体のスタイル（画面中央に固定表示、背景を暗く）
        ...
        modalContent.id = 'koto-map-modal-content';
        ...
        // スクロール可能なデータ表示エリア
        var bodyArea = document.getElementById('koto-map-content');
        if (!bodyArea) {
            bodyArea = document.createElement('div');
            bodyArea.id = 'koto-map-content';
        }
    ...
    // 3. データ（Map）の生成・追加処理
    var contentTarget = document.getElementById('koto-map-content');
    var fragment = document.createDocumentFragment();

    // オブジェクトから各配列を統合
    var allMapLists = [];
    Object.keys(window.kotoMaps).forEach(function (key) {
        if (Array.isArray(window.kotoMaps[key])) {
            allMapLists = allMapLists.concat(window.kotoMaps[key]);
        }
    });

    // 各要素を生成して仮想コンテナに詰める
    allMapLists.forEach(function (map) {
...
// Map読み込みボタンのクリックイベント（行複製時にも対応するため delegation で設定）
$(document).on('click', '.koto-load-map-btn', function (e) {
    e.preventDefault();
    var $currentRow = $(this).closest('.acf-row');
...
acf.addAction('new_field/name=attack_prefix', function (field) {
    // 既にボタンが存在する場合は追加しない（複製時の重複追加を防止）
    if (field.$el.siblings('.koto-load-map-btn').length === 0) {
        // Map読み込みボタンを追加
    ...
    if (window.kotoSharedActiveMap) {
        ...
        if (hasAttackType($currentRow)) {
            ...
            console.log('❌ Auto-fill skipped. Type does not include attack.');
        }
    } else {
    ...
    field.on('change', function () {
        var $currentRow = field.$el.closest('.acf-row');
        if (!hasAttackType($currentRow)) return;
...
acf.addAction('new_field/name=waza_value', function (field) {
    // EnterキーでMap読み込みを試行し、見つからない場合のみ「自動入力を使用しない」に切り替え
...
acf.addAction('new_field/name=multi_cond_type', function (field) {
    field.on('change', function () {
        const selectedValue = field.val();

        // --- ここからダミーコード ---

        // ★★★ 要設定: multi_cond_type の値に応じて生成するデータを定義します。
        // キーは multi_cond_type の選択肢の値、値は追加したい行のデータ配列です。
        const dataMap = {
            'enemy': [
                { enemy_count: '1' },
                { enemy_count: '2' },
            ...
            'both': [
                { enemy_count: '1', moji_count: '4' },
                { enemy_count: '1', moji_count: '5' },
                { enemy_count: '1', moji_count: '6' },
                { enemy_count: '1', moji_count: '7' },
                { enemy_count: '2', moji_count: '4' },
                { enemy_count: '2', moji_count: '5' },
                { enemy_count: '2', moji_count: '6' },
                { enemy_count: '2', moji_count: '7' },
                { enemy_count: '3', moji_count: '4' },
                { enemy_count: '3', moji_count: '5' },
                { enemy_count: '3', moji_count: '6' },
                { enemy_count: '3', moji_count: '7' },
            ],
            'target': [] // 「なし」や未選択の場合は空の配列にしておくと、行がクリアされます。
        };

        // ★★★ 要設定: データを投入したい繰り返しフィールドの「フィールド名」を指定します。
        const repeaterFieldName = 'maltiplier_table';

        // --- ダミーコードここまで ---

        const dataToGenerate = dataMap[selectedValue] || [];
        const $repeater = $('.acf-field[data-name="' + repeaterFieldName + '"]:visible');

        if (!$repeater.length) {
            console.error('繰り返しフィールド "' + repeaterFieldName + '" が見つかりません。フィールド名が正しいか確認してください。');
            return;
        }
        ...
        // 2. 生成するデータをキュー（順番待ちリスト）に入れます。
        let dataQueue = [...dataToGenerate].reverse();

        // 3. ACFで行が追加された（append）時のイベントリスナーを定義します。
        const eventListener = function ($el) {
            if ($el.closest('.acf-field').data('name') !== repeaterFieldName) return;

            const currentData = dataQueue.shift();
            if (typeof currentData === 'undefined') {
                return setTimeout(() => acf.removeAction('append', eventListener), 0);
            }

            // 4. ★★★ 要設定: ここの 'condition', 'multiplier' を実際のサブフィールド名に書き換えてください。
            const enemyFields = acf.getFields({ name: 'enemy_count', parent: $el });
            if (enemyFields.length) enemyFields[0].val(currentData.enemy_count || '');

            const mojiFields = acf.getFields({ name: 'moji_count', parent: $el });
            if (mojiFields.length) mojiFields[0].val(currentData.moji_count || '');

            // 5. キューにまだデータが残っていれば、次の行を追加します。
            if (dataQueue.length > 0) $repeater.find('[data-event="add-row"]').first().trigger('click');
            else setTimeout(() => acf.removeAction('append', eventListener), 0);
        };

        acf.addAction('append', eventListener);
        $repeater.find('[data-event="add-row"]').first().trigger('click');
    });
});

// コトわざ自動入力欄のトグル処理
const btnToggleKotowaza = document.getElementById('btn_toggle_kotowaza');
const kotowazaArea = document.getElementById('kotowaza_auto_input_area');

if (btnToggleKotowaza && kotowazaArea) {
    btnToggleKotowaza.addEventListener('click', function () {
        // CSSで指定された現在のdisplayプロパティを取得して判定
        const currentDisplay = window.getComputedStyle(kotowazaArea).display;

        if (currentDisplay === 'none') {
            kotowazaArea.style.display = 'flex';
            btnToggleKotowaza.textContent = 'コトわざ自動入力欄を閉じる';
        } else {
            kotowazaArea.style.display = 'none';
            btnToggleKotowaza.textContent = 'コトわざ自動入力欄を展開';
        }
    });
}