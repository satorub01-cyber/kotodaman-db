document.addEventListener('DOMContentLoaded', function () {
    // Xdebug トグルボタンの生成（ローカル環境のみ）
    if (window.AppDebugConfig && window.AppDebugConfig.isLocal) {
        let isXdebugActive = localStorage.getItem('app_xdebug_active') === 'true';

        const btnToggle = document.createElement('button');
        btnToggle.type = 'button';
        btnToggle.className = 'xdebug-toggle-btn' + (isXdebugActive ? ' is-active' : '');
        btnToggle.textContent = isXdebugActive ? 'Xdebug: ON' : 'Xdebug: OFF';

        btnToggle.addEventListener('click', function () {
            isXdebugActive = !isXdebugActive;
            localStorage.setItem('app_xdebug_active', isXdebugActive);
            btnToggle.textContent = isXdebugActive ? 'Xdebug: ON' : 'Xdebug: OFF';
            btnToggle.classList.toggle('is-active', isXdebugActive);
        });

        const container = document.createElement('div');
        container.className = 'xdebug-toggle-container';
    ...
    // Ajaxリクエスト用 URL取得関数
    function getAjaxUrl() {
        const base = (window.AppDebugConfig && window.AppDebugConfig.ajaxUrl) ? window.AppDebugConfig.ajaxUrl : ajaxurl;
        const isXdebugActive = localStorage.getItem('app_xdebug_active') === 'true';

        if (window.AppDebugConfig?.isLocal && isXdebugActive) {
            const separator = base.indexOf('?') === -1 ? '?' : '&';
            return base + separator + 'XDEBUG_SESSION_START=VSCODE';
        }
        return base;
    }
    ...
    function getAutoInputData() {
        ...
        return {
            ...
            // auto-input-追加必須: HTML側の入力欄を texts に追加
            texts: {
                ...
                auto_input_leader: getSanitizedText('auto_input_leader'),
