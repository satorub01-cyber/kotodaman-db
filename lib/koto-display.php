...
function get_koto_sugowaza_html($group_data, $condition_data = null, $skill_type = '', $attack = 0)
{
    ...
    if ($group_data && is_array($group_data)) {
        ...
        foreach ($grouped_data as $group_info) {
                ...
                if (!empty($eff_type)) {
                    switch ($eff_type) {
                        ...
                        case 'converged_attack':
                            ...
                            if ($hit_count > 1) {
                                if ($val_last > 0) {
                                    ...
                                    $current_group_at_rate_array[] = ['rate' => $raw_eff_val, 'hit_count' => $loop];
                                    $current_group_at_rate_array[] = ['rate' => (float)$val_last, 'hit_count' => 1];
                                } else {
                                    $eff_val = "（{$eff_val}×{$hit_count}）";
                                    $current_group_at_rate_array[] = ['rate' => $raw_eff_val, 'hit_count' => $hit_count];
                                }
                            }
                            if ($rate_range_flag) {
                        ...
                        case 'colorfull_attack':
                            ...
                            $current_group_at_rate_array[] = ['rate' => (float)$eff_val, 'hit_count' => count($order_names)];
                            break;
                        case 'coop_attack':
                            ...
                            $current_group_at_rate_array[] = ['rate' => (float)$eff_val, 'hit_count' => 3];
                            break;
                        case 'command':
                            $effect_text = "わざ・すごわざを発動した味方が敵単体に{$eff_val}倍の{$attack_attr}属性攻撃";
            ...
            $group_at_rate_html = '';
            $group_at_sum = 0;
            if ($current_group_at_rate_array) {
                $at_rate_parts = [];
                foreach ($current_group_at_rate_array as $item) {
                    ...
                    $group_at_sum += $firepower * $item['hit_count'];
                }
...
function koto_replace_icons($buffer)
{
    ...
    $buffer = preg_replace_callback($pattern_attr, function ($matches) use ($slug_map) {
        ...
        // 軽量版WebP (150x150) を指定
        $icon_url = 'https://www.kotodaman-db.com/wp-content/uploads/2025/12/icon-' . $slug . '-150x150.png.webp';

        // width/heightを指定してCLSを防止
        $img_tag = '<img src="' . $icon_url . '" alt="' . $element_name . '属性" class="attr-icon-img" width="29" height="29" loading="lazy">';

        return ($suffix === '・') ? $img_tag . '・' : $img_tag;
    }, $buffer);
    ...
    $buffer = preg_replace_callback($pattern_race, function ($matches) use ($slug_map_race) {
        ...
        $img_tag = '<img src="' . $icon_url . '" alt="' . $name . '種族" class="species-icon-img" width="29" height="29" loading="lazy">';
