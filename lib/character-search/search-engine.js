...
const getActiveAndSearchKey = () => {
    if (window._isCharAndSearch) return 'char';
    if (window._isGimmickAndSearch) return 'gimmick';
    return null;
};

const getActiveMatchKey = () => {
    if (window._isCharAndSearch) return 'char';
    if (window._hasGimmickSearch) return 'gimmick';
    return null;
};
...
function sortCharacters(chars) {
    return chars.sort((a, b) => {
        // ★第0ソート：検索一致数（降順）- AND検索時のみ
        const activeMatchKey = getActiveMatchKey();
        if (activeMatchKey) {
            const scoreA = getMatchCount(a, activeMatchKey);
            const scoreB = getMatchCount(b, activeMatchKey);
            if (scoreA !== scoreB) {
...
function renderTable(chars) {
    ...
    if (chars.length === 0) {
        ...
        const activeAndSearchKey = getActiveMatchKey();

        if (activeAndSearchKey) {
        // ★AND検索時：セクション区切り行を挿入して表示
        const uniqueCounts = [...new Set(chars.map(c => getMatchCount(c, activeAndSearchKey) || 0))].sort((a, b) => b - a);
        console.log('[DEBUG renderTable] uniqueCounts:', uniqueCounts);

        let html = '';
        uniqueCounts.forEach(count => {
            const sectionChars = chars.filter(c => getMatchCount(c, activeAndSearchKey) === count);
            // セクションヘッダー行（tr）として追加
            html += `<tr class="section-header" style="background-color: #e3f2fd;">
                <td colspan="100%" style="padding: 10px; font-weight: bold; color: #1976d2; border-top: 2px solid #1976d2;">
                    ${activeAndSearchKey === 'char' ? `${count}文字含む` : `${count}件のギミック一致`}キャラクター (${sectionChars.length}件)
                </td>
            </tr>`;
            // セクション内のキャラクター
