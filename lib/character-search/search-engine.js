...
function createCharacterRowHtml(char) {
    ...
    // メイン属性（画像URLがあれば img タグ、なければ従来の span タグ）
    let attrHtml = attrData.icon
        ? `<img src="${attrData.icon}" class="attr-icon-img" alt="${attrData.name}">`
        : `<span class="attr-text attr-${attrData.slug}">${attrData.name}</span>`;
    ...
    let speciesHtml = speciesData.icon
        ? `<img src="${speciesData.icon}" class="species-icon-img" alt="${speciesData.name}">`
        : `<span class="attr-text attr-${speciesData.slug}">${speciesData.name}</span>`;
    let gimmickHtml = '';
    ...
    if (char.grp && char.grp.length > 0) {
        groupHtml = char.grp.map(slug => {
            // URLエンコードされている(%e9%ac...)場合があるので安全にデコードする
            let decoded = slug;
            try { decoded = decodeURIComponent(slug); } catch (e) { }
            // 辞書にあれば日本語名、なければそのまま出力
            return KOTO_GROUP_MAP[decoded] || decoded;
        }).join('<br>'); // 複数ある場合は改行で並べる
    }
    ...
    if (char.events && char.events.length > 0) {
        eventHtml = char.events.map(slug => {
            let decoded = slug;
            try { decoded = decodeURIComponent(slug); } catch (e) { }
            return KOTO_EVENT_MAP[decoded] || decoded;
        }).join('<br>');
    }
