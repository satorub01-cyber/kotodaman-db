...
function updateSearchUrlAndAnalytics() {
    ...
    // フォームの中身を1つずつチェックしてURLとGA4データに振り分ける
    for (const [key, value] of formData.entries()) {
        // 空の入力や、システム用の一部のhiddenはURLに入れない
        if (value.trim() !== '' && key !== 'post_type') {
            urlParams.append(key, value);

            // GA4用にデータを振り分け
            if (key === 's') gaData.search_term = value;
            else if (key === 'tx_attr[]') gaData.filter_attr.push(value);
    ...
    // ② 画面をリロードせずにURL（アドレスバー）だけを書き換える
    const queryString = urlParams.toString();
    const newUrl = window.location.pathname + (queryString ? '?' + queryString : '');
    // history.replaceState は、ブラウザの「戻る」履歴を増やさずにURLだけを上書きするメソッドです
